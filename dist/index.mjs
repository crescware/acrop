...
function processSiblingRule(rule, tsPath, root) {
  if (!("sibling" in rule)) {
    return null;
  }
...
function processAllowedRule(rule, relativePath) {
  if (!("allowed" in rule)) {
    return [];
  }
  const paths = (() => {
...
function processRestrictedRule(rule, relativePath) {
...
// src/check/check.ts
function check(logger, config, tsFiles, root) {
  ...
  for (const declaration of config.scopes) {
    ...
    const filtered = tsFiles.filter((tsFile) => {
      ...
      const ret = minimatch2(tsFile.relative, declaration.scope);
      end3();
      return ret;
    });
    end2();
    for (const path of filtered) {
      ...
      const infoArray = findImportPaths(ast, positions).map(
        (v) => {
          const relativePath = `./${relative2(root, resolve2(dirname2(path.absolute), v.path.relative))}`;
          return {
